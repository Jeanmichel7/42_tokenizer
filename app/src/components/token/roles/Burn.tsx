...
const Burn = ({ contractTokenWithSigner }: BurnProps) => {
  const [error, setError] = useState<string>("");
  const [txId, setTxId] = useState<string>("");

  const [form, setForm] = useState<formBurn>({
    address: "",
    amount: "",
  });
  ...
  const handleMint = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const ret = await contractTokenWithSigner.burn(address, amount);
      setTxId(ret.hash);
  ...
  return (
    <div className='w-1/2'>
      <h2 className='text-center text-lg font-bold mt-5'>Burn</h2>
      <div
