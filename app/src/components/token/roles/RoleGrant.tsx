...
const RoleGrant = ({ contractTokenWithSigner }: RoleGrantProps) => {
  ...
  const handleGrantRole = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const ret = await contractTokenWithSigner.grantRole(roleBytes32, address);
      setTxId(ret.hash);
  ...
  return (
    <div className='w-[66vw]'>
      <h2 className='text-center text-lg font-bold mt-5'>Grant Role</h2>
      <div
